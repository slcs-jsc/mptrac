...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{   
   double Ynew[3], Fcn0[3], Fcn[3],
      dFdT[3],
      Jac0[7], Ghimj[7];
   double K[3*ros_S];   
   double H, T, Hnew, HC, HG, Fac, Tau; 
   double Err, Yerr[3];
   int Pivot[3], Direction, ioffset, j, istage;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  ...
  /*~~~>   Compute the stages  */
   for (istage = 1; istage <= ros_S; istage++) {
      ...
      /* Current istage offset. Current istage vector is K[ioffset:ioffset+3-1] */
      ioffset = 3*(istage-1);
	 
      /* For the 1st istage the function has been computed previously */
      if ( istage == 1 )
	   WCOPY(3,Fcn0,1,Fcn,1);
      else { /* istage>1 and a new function evaluation is needed at current istage */
	if ( ros_NewF[istage-1] ) {
	   WCOPY(3,Y,1,Ynew,1);
	   for (j = 1; j <= istage-1; j++)
	     WAXPY(3,ros_A[(istage-1)*(istage-2)/2+j-1],
                   &K[3*(j-1)],1,Ynew,1); 
  ...
  /*~~~>  Compute the new solution   */
   WCOPY(3,Y,1,Ynew,1);
   for (j=1; j<=ros_S; j++)
       WAXPY(3,ros_M[j-1],&K[3*(j-1)],1,Ynew,1);

  /*~~~>  Compute the error estimation   */
   WSCAL(3,ZERO,Yerr,1);
   for (j=1; j<=ros_S; j++)    
       WAXPY(3,ros_E[j-1],&K[3*(j-1)],1,Yerr,1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/   
{
  /*~~~> Local variables */     
   ...
   WAXPY(3,(-ONE),Fcn0,1,dFdT,1);
   WSCAL(3,(ONE/Delta),dFdT,1);
...
{   
  ...
  /*~~~> Note: for a full matrix use Lapack:
      NRHS = 1
      DGETRS( 'N', 3 , NRHS, A, 3, Pivot, b, 3, INFO ) */
