...
double clim_oh(
  ...
  aux00 = LIN(clim->oh_time[isec], aux00, clim->oh_time[isec + 1], aux11, sec);
...
void clim_oh_init(
  ...
  /* Write info... */
  printf("Read OH data: %s", filename);
  ...
  /* TODO: Please add a check to see the pressure data are in correct order,
  ...
  /* TODO: Please add a check to see the latitude data are in correct order,
  ...
  /* TODO: Please add a check to see whether the OH netCDF data files provides
     monthly data, i.e., make sure there are 12 (= CT) time steps in the
     data file. */

  
  /* Read OH data... */
  NC(nc_inq_varid(ncid, "OH", &varid));
  ALLOC(help, double, clim->oh_ny * clim->oh_np * clim->oh_nt);
  NC(nc_get_var_double(ncid, varid, help));
  for (ix = 0; ix < clim->oh_nt; ix++)
    for (iy = 0; iy < clim->oh_np; iy++)
      for (iz = 0; iz < clim->oh_ny; iz++)
	clim->oh[ix][iy][iz] =
	  help[ix * clim->oh_np * clim->oh_ny + iy * clim->oh_ny + iz];
