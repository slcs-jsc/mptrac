...
void compress_log_level(
  ...
  double nrmse = NAN, mean_orig = 0.0, orig_range = 0.0, rho = NAN;
  ...
  /* Accumulate error, signal and cross-product sums in one pass... */
  ...
  rho = denom > 0.0 ? cov / denom : NAN;

  /* Reset the variable separator state when the target file changes... */
  if (out != last_out) {
    last_out = out;
    last_var[0] = '\0';
  }

  /* Separate diagnostic blocks when switching to a new variable... */
  ...
  fprintf(out,
	  ...
	  codec, varname, (unsigned long) lev, plev, ratio, bpv, rho,
...
void compress_log_levels_3d(
  ...
  /* Allocate temporary buffers for one level... */
  float *tmp_org, *tmp_cmp;
  ALLOC(tmp_org, float,
	nxy);
  ALLOC(tmp_cmp, float,
	nxy);
...
#pragma omp parallel for default(shared)
    for (size_t ixy = 0; ixy < nxy; ixy++) {
      tmp_org[ixy] = org_all[ixy * nz + lev];
      tmp_cmp[ixy] = cmp_all[ixy * nz + lev];
    }

    /* Write diagnostics for the extracted level... */
...
void compress_scale_to_unit(
  ...
  /* Initialize per-level minima and maxima from the first sample... */
  for (size_t iz = 0; iz < nz; iz++) {
    off[iz] = array[iz];
    scl[iz] = array[iz];
  }

  /* Scan the remaining samples to get per-level minima and maxima... */
  for (size_t ixy = 1; ixy < nxy; ixy++)
    for (size_t iz = 0; iz < nz; iz++) {
      const double value = array[ixy * nz + iz];
      if (value < off[iz])
	off[iz] = value;
      if (value > scl[iz])
	scl[iz] = value;
  ...
  /* Convert maxima into ranges and suppress numerically flat levels... */
  for (size_t iz = 0; iz < nz; iz++) {
    const double range = scl[iz] - off[iz];
    const double ref = fmax(1.0, fmax(fabs(off[iz]), fabs(scl[iz])));
    scl[iz] = !(range > 1e-12 * ref) ? 0.0 : range;
  }

  /* Scale each level independently to the unit interval... */
#pragma omp parallel for default(shared)
  for (size_t ixy = 0; ixy < nxy; ixy++)
    for (size_t iz = 0; iz < nz; iz++)
      if (scl[iz] > 0.0)
	array[ixy * nz + iz] =
	  (float) ((array[ixy * nz + iz] - off[iz]) / scl[iz]);
      else
	array[ixy * nz + iz] = 0.0f;
}
...
void compress_unscale_from_unit(
  ...
  /* Restore each level from unit space back to its original range... */
#pragma omp parallel for default(shared)
  for (size_t ixy = 0; ixy < nxy; ixy++)
    for (size_t iz = 0; iz < nz; iz++)
      if (scl[iz] > 0.0)
	array[ixy * nz + iz] =
	  (float) (array[ixy * nz + iz] * scl[iz] + off[iz]);
      else
	array[ixy * nz + iz] = (float) off[iz];
}
...
ZSTD_CCtx *compress_zstd_create_cctx(
  ...
  /* Create and configure the ZSTD context... */
...
int compress_metvar_index(
  ...
  for (size_t i = 0; i < sizeof(names) / sizeof(names[0]); i++)
    if (strcasecmp(varname, names[i]) == 0)
      return (int) i;

  ERRMSG("Unknown meteorological variable name!");
}
...
void compress_pck(
  ...
  /* PCK+ZSTD payload = scale + offset + packed 16-bit samples... */
  ...
  if (decompress) {
    ...
    if (ctl->met_pck_zstd) {
#ifdef ZSTD
      uint64_t magic, pck_zstd_magic = UINT64_C(0x50434b5a53544431);
	...
	ERRMSG("PCK+ZSTD magic mismatch! Check MET_PCK_ZSTD and file type.");
      ...
      /* Read and decompress the outer ZSTD payload... */
	...
	ZSTD_decompress(payload, payload_len, compr, stored_len);
      t_zstd = omp_get_wtime() - t0_decomp;
      if (ZSTD_isError(out_len) || out_len != payload_len)
	ERRMSG("PCK ZSTD decompression failed or size mismatch!");

      /* Split payload into scale, offset and packed samples... */
    ...
    /* Measure the scalar unpacking step separately from optional ZSTD... */
    const double t0 = omp_get_wtime();

    /* Convert packed 16-bit samples back to floating-point values... */
#pragma omp parallel for default(shared)
    for (size_t ixy = 0; ixy < nxy; ixy++)
      for (size_t iz = 0; iz < nz; iz++)
	array[ixy * nz + iz]
    ...
    /* Add optional ZSTD time to the scalar unpacking time... */
    const double t_decomp = t_zstd + omp_get_wtime() - t0;

    /* Logging... */
    LOG(2, "Read 3-D variable: %s"
	" (%s, RATIO=%g, BPV=%g, T_DECOMP=%g s, V_DECOMP=%g MiB/s)",
	varname, codec_info, COMPRESS_RATIO(nbytes, stored_len),
	COMPRESS_BPV(n, stored_len), t_decomp, COMPRESS_SPEED(nbytes,
							      t_decomp));
  }
  ...
  /* Compress array and output compressed stream... */
    ...
    /* Derive per-level minima and maxima for the 16-bit quantization... */
    for (size_t iz = 0; iz < nz; iz++) {
      vmin[iz] = array[iz];
      vmax[iz] = array[iz];
    }
    for (size_t ixy = 1; ixy < nxy; ixy++)
      for (size_t iz = 0; iz < nz; iz++) {
	if (array[ixy * nz + iz] < vmin[iz])
	  vmin[iz] = array[ixy * nz + iz];
	if (array[ixy * nz + iz] > vmax[iz])
	  vmax[iz] = array[ixy * nz + iz];
      }

    /* Derive per-level offset and scaling factor... */
    for (size_t iz = 0; iz < nz; iz++) {
      scl[iz] = (vmax[iz] - vmin[iz]) / 65533.;
      off[iz] = vmin[iz];
    }

    /* Quantize each level and time encode/decode diagnostics on the fly... */
    for (size_t iz = 0; iz < nz; iz++) {
      const double t0 = omp_get_wtime();
...
#pragma omp parallel for default(shared)
    ...
    /* Write packed data, optionally wrapped in ZSTD... */
    if (ctl->met_pck_zstd) {
#ifdef ZSTD
      uint64_t pck_zstd_magic = UINT64_C(0x50434b5a53544431);
      unsigned char *payload = NULL, *payload_chk = NULL;
      void *stored_data = NULL;

      /* Pack plain PCK data into one ZSTD payload... */
      ALLOC(payload, unsigned char,
	    payload_len);
	...
	ZSTD_compress2(cctx, stored_data, dst_cap, payload, payload_len);
      t_zstd = omp_get_wtime() - t0_comp;
      ZSTD_freeCCtx(cctx);
      if (ZSTD_isError(stored_len))
	ERRMSG("PCK ZSTD compression failed!");

      /* Decompress once for validation and timing... */
      ALLOC(payload_chk, unsigned char,
	    payload_len);
      const double t0_decomp2 = omp_get_wtime();
      const size_t out_len2 =
	ZSTD_decompress(payload_chk, payload_len, stored_data, stored_len);
      t_zstd_decomp = omp_get_wtime() - t0_decomp2;
      if (ZSTD_isError(out_len2) || out_len2 != payload_len)
	ERRMSG("PCK ZSTD decompression failed or size mismatch!");
    ...
    /* Combine PCK and optional ZSTD timing and size metrics... */
	...
	" T_DECOMP=%g s, V_DECOMP=%g MiB/s)",
	varname, codec_info, ratio_out, bpv_out, t_comp,
	COMPRESS_SPEED(nbytes, t_comp), t_decomp,
	COMPRESS_SPEED(nbytes, t_decomp));

    /* Distribute optional ZSTD cost across levels for diagnostics... */
    if (level_log) {
...
#pragma omp parallel for default(shared)
	...
	/* Rebuild one decoded level for diagnostics... */
	for (size_t ixy = 0; ixy < nxy; ixy++) {
	  tmp_org[ixy] = array[ixy * nz + iz];
	  tmp_pck[ixy] = (float) (sarray[ixy * nz + iz] * scl[iz] + off[iz]);
	}
...
void compress_sz3(
  ...
  if ((ctl->met_sz3_prec[metvar] > 0) == (ctl->met_sz3_tol[metvar] > 0.0))
    ERRMSG("Exactly one of precision or tolerance must be set for SZ3!");

  size_t r1 = (size_t) met->np, r2 = (size_t) met->ny, r3 = (size_t) met->nx,
  ...
  if (decompress) {
    ...
    /* Read optional scaling metadata before the SZ3 byte stream... */
    ...
    /* Let SZ3 rebuild the scaled field in its native 3-D layout... */
    const double t0 = omp_get_wtime();
    ...
    /* Restore the original value range after lossy unit scaling... */
    if (stored_lossy_scale > 0)
      compress_unscale_from_unit(array, r2 * r3, r1, scale_off, scale_scl);
    ...
    const double ratio = COMPRESS_RATIO(total_elems * sizeof(float), sz3size);
    const double bpv = COMPRESS_BPV(total_elems, sz3size);
  ...
  /* Compress array and output compressed stream... */
    ...
    /* Translate MPTRAC settings into the SZ3 absolute/relative error mode... */
    const int errBoundMode = (ctl->met_sz3_prec[metvar] > 0) ? REL : ABS;
    const double absBound =
      (errBoundMode == ABS) ? ctl->met_sz3_tol[metvar] : 0.0;
    const double relBound =
      (errBoundMode == REL) ? pow(2.0, -(double) ctl->met_sz3_prec[metvar])
      : 0.0;
    const size_t raw_size = total_elems * sizeof(float);

    /* Preserve the unscaled input only if diagnostics need it later... */
    ...
    /* Optionally scale each vertical level to unit space before SZ3... */
    compress_write_lossy_scale(inout, ctl->met_lossy_scale[metvar], array,
			       r2 * r3, r1, &scale_off, &scale_scl);

    /* Compress the possibly scaled field with the selected SZ3 error mode... */
    const double t0 = omp_get_wtime();
    ...
    /* Reconstruct once more only when per-level diagnostics are requested... */
    double t_decomp = NAN;
    if (level_log) {
	...
	ERRMSG("Decompression failed!");
      ...
      /* Rebuild the decoded field so diagnostics compare like with like... */
      ALLOC(tmp_all, float,
	    total_elems);
      memcpy(tmp_all, decData, total_elems * sizeof(float));
      if (ctl->met_lossy_scale[metvar] > 0)
	compress_unscale_from_unit(tmp_all, r2 * r3, r1, scale_off,
				   scale_scl);

      const double ratio = COMPRESS_RATIO(raw_size, outSize);
      const double bpv = COMPRESS_BPV(total_elems, outSize);
      compress_log_levels_3d(level_log, "SZ3", varname, met,
			     (orig_all ? orig_all : array), tmp_all, nxy, r1,
			     ratio, bpv, t_comp, t_decomp, raw_size);
	  ...
	  COMPRESS_SPEED(raw_size, t_comp), t_decomp,
...
void compress_zfp(
  ...
  const size_t snx = (size_t) met->np;
  const size_t sny = (size_t) met->ny;
  const size_t snz = (size_t) met->nx;
  const size_t n = snx * sny * snz;
  const size_t raw_size = n * sizeof(float);
  ...
  int actual_prec = 0;
  double actual_tol = 0;
  if ((ctl->met_zfp_prec[metvar] > 0 && ctl->met_zfp_tol[metvar] > 0)
      || (ctl->met_zfp_prec[metvar] <= 0 && ctl->met_zfp_tol[metvar] <= 0)) {
    ERRMSG("Exactly one of precision or tolerance must be set for zfp!");
  } else if (ctl->met_zfp_prec[metvar] > 0)
    actual_prec = (int) zfp_stream_set_precision(zfp, (unsigned int)
						 ctl->met_zfp_prec[metvar]);
  else if (ctl->met_zfp_tol[metvar] > 0)
    actual_tol = zfp_stream_set_accuracy(zfp, ctl->met_zfp_tol[metvar]);
  ...
  if (decompress) {
    ...
    /* Read optional scaling metadata before the ZFP bit stream... */
    const int stored_lossy_scale =
    ...
    /* Decode the bit stream directly into the caller-provided field... */
    const double t0 = omp_get_wtime();
    ...
    /* Restore the original value range after lossy unit scaling... */
    if (stored_lossy_scale > 0)
      compress_unscale_from_unit(array, sny * snz, snx, scale_off, scale_scl);
    const double t_decomp = omp_get_wtime() - t0;
    const double ratio = COMPRESS_RATIO(raw_size, zfpsize);
    const double bpv = COMPRESS_BPV(n, zfpsize);
    ...
    LOG(2, "Read 3-D variable: %s"
	" (%s, RATIO=%g, BPV=%g, T_DECOMP=%g s, V_DECOMP=%g MiB/s)",
	varname, codec_info, ratio, bpv, t_decomp, COMPRESS_SPEED(raw_size,
								  t_decomp));
  }

  /* Compress array and output compressed stream... */
  else {
    float *tmp_all, *orig_all = NULL;
    ...
    if (ctl->met_lossy_scale[metvar] > 0 && level_log) {
      ALLOC(orig_all, float,
	    n);
      memcpy(orig_all, array, raw_size);
    }

    /* Optionally scale each vertical level to unit space before ZFP... */
    compress_write_lossy_scale(inout, ctl->met_lossy_scale[metvar], array,
			       nxy, snx, &scale_off, &scale_scl);

    /* Keep a temporary decode buffer for the shared diagnostics path... */
    ALLOC(tmp_all, float,
	  n);

    /* Compress the field into the configured ZFP bit stream... */
    const double t0 = omp_get_wtime();
    ...
    /* Decode once into a temporary field for the shared diagnostics... */
    zfp_field *dec_field = zfp_field_3d(tmp_all, type, snx, sny, snz);
    if (!dec_field)
      ERRMSG("Failed to allocate zfp structures!");

    /* Rewind the bit stream and decode once for diagnostics and timing... */
    const double t1 = omp_get_wtime();
    zfp_stream_rewind(zfp);
    if (!zfp_decompress(zfp, dec_field))
      ERRMSG("Decompression failed!");
    if (ctl->met_lossy_scale[metvar] > 0)
      compress_unscale_from_unit(tmp_all, nxy, snx, scale_off, scale_scl);
	...
	" (%s, RATIO=%g, BPV=%g, T_COMP=%g s, V_COMP=%g MiB/s,"
	" T_DECOMP=%g s, V_DECOMP=%g MiB/s)",
	varname, codec_info, ratio, bpv, t_comp, COMPRESS_SPEED(raw_size,
								t_comp),
	t_decomp, COMPRESS_SPEED(raw_size, t_decomp));
...
void compress_zstd(
  ...
  const size_t nxy = (size_t) met->nx * (size_t) met->ny;
  const size_t nz = (size_t) met->np;
  const size_t n = nxy * nz, uncomprLen = n * sizeof(float);
  ...
  if (decompress) {
    ...
    /* Decode one full frame and validate the restored byte count... */
    const double t0 = omp_get_wtime();
    ...
    const double ratio = COMPRESS_RATIO(uncomprLen, comprLen);
    const double bpv = COMPRESS_BPV(n, comprLen);
    snprintf(codec_info, LEN, "ZSTD, LEVEL=%d", ctl->met_zstd_level);

    /* Logging... */
    LOG(2, "Read 3-D variable: %s"
	" (%s, RATIO=%g, BPV=%g, T_DECOMP=%g s, V_DECOMP=%g MiB/s)",
	varname, codec_info, ratio, bpv, t_decomp, COMPRESS_SPEED(uncomprLen,
								  t_decomp));
  }
  ...
  /* Compress array and output compressed stream... */
    ...
    /* Keep one temporary decode buffer for the shared diagnostics path... */
    ALLOC(tmp_all, float,
	  n);
    ...
    /* Compress one full field with the preconfigured ZSTD context... */
    const double t0 = omp_get_wtime();
    ...
    /* Decode once more so the common diagnostics see the reconstructed field... */
    const double t1 = omp_get_wtime();
    ...
    const double ratio = COMPRESS_RATIO(uncomprLen, compsize);
    const double bpv = COMPRESS_BPV(n, compsize);
    compress_log_levels_3d(level_log, "ZSTD", varname, met, array, tmp_all,
			   nxy, nz, ratio, bpv, t_comp, t_decomp, uncomprLen);
	...
	" (%s, RATIO=%g, BPV=%g, T_COMP=%g s, V_COMP=%g MiB/s,"
	" T_DECOMP=%g s, V_DECOMP=%g MiB/s)",
	varname, codec_info, ratio, bpv, t_comp, COMPRESS_SPEED(uncomprLen,
								t_comp),
	t_decomp, COMPRESS_SPEED(uncomprLen, t_decomp));
...
void compress_lz4(
  ...
  const int accel = ctl->met_lz4_accel > 0 ? ctl->met_lz4_accel : 1;
  ...
  /* Compress array and output compressed stream... */
    ...
    /* Compress one full field with the configured LZ4 acceleration... */
    const double t0 = omp_get_wtime();
...
void dd_assign_subdomains(
  ...
  /* Determine owned core subdomain in index space... */
  const int zonal_rank = rank / ctl->dd_subdomains_meridional;
  const int merid_rank = rank % ctl->dd_subdomains_meridional;

  const int nx_block = dd->nx_glob / ctl->dd_subdomains_zonal;
  const int ny_block = dd->ny_glob / ctl->dd_subdomains_meridional;

  const int ix0 = zonal_rank * nx_block;
  const int iy0 = merid_rank * ny_block;

  const int ix1 = ix0 + nx_block
    + (zonal_rank == ctl->dd_subdomains_zonal - 1 ?
       dd->nx_glob - ctl->dd_subdomains_zonal * nx_block : 0);

  const int iy1 = iy0 + ny_block
    + (merid_rank == ctl->dd_subdomains_meridional - 1 ?
       dd->ny_glob - ctl->dd_subdomains_meridional * ny_block : 0);

  /* Loop over particles... */
#ifdef _OPENACC
#pragma acc update device(dd->nx_glob, dd->ny_glob, \
...
#pragma acc parallel loop independent gang vector
  ...
  for (int ip = 0; ip < atm->np; ip++) {
    ...
    /* Check whether particle is inside owned core subdomain... */
    const int ix = locate_reg(dd->lon_glob, dd->nx_glob, lon);
    const int iy = locate_irr(dd->lat_glob, dd->ny_glob, lat);
    const int inside = (ix >= ix0 && ix < ix1 && iy >= iy0 && iy < iy1);
...
int dd_calc_subdomain_from_coords(
  ...
  /* Get zonal subdomain index... */
  const int nx_block = dd->nx_glob / ctl->dd_subdomains_zonal;
  int zonal_rank = ix / nx_block;
  if (zonal_rank >= ctl->dd_subdomains_zonal)
    zonal_rank = ctl->dd_subdomains_zonal - 1;

  /* Get meridional subdomain index... */
  const int ny_block = dd->ny_glob / ctl->dd_subdomains_meridional;
  int merid_rank = iy / ny_block;
  if (merid_rank >= ctl->dd_subdomains_meridional)
    merid_rank = ctl->dd_subdomains_meridional - 1;

  /* Return rank... */
  return zonal_rank * ctl->dd_subdomains_meridional + merid_rank;
}
#endif
...
void dd_communicate_particles(
  ...
  particle_t *sendbuf = NULL;
  particle_t *recvbuf = NULL;

  ALLOC(send_counts, int,
	size);
  ALLOC(recv_counts, int,
	size);
  ALLOC(send_displs, int,
	size);
  ALLOC(recv_displs, int,
	size);
  ALLOC(offsets, int,
	size);
...
void dd_init(
  ...
  int size;
  MPI_Comm_size(MPI_COMM_WORLD, &size);
  if (size != ctl->dd_subdomains_meridional * ctl->dd_subdomains_zonal)
...
#ifdef _OPENACC
#pragma acc enter data create(amax)
#pragma acc update device(amax)
#pragma acc data present(ctl,met0,atm,dd,amax)
#endif
...
#pragma acc update device(atm->np)
  ...
  /* Free... */
#ifdef _OPENACC
#pragma acc exit data delete(amax)
#endif
}
#endif
...
void mptrac_read_ctl(
...
#endif
  const int def_lossy_scale =
    (int) scan_ctl(filename, argc, argv, "MET_LOSSY_SCALE", -1, "0", NULL);
  for (int i = 0; i < METVAR; i++) {
    ...
    char defscale[LEN];
    snprintf(defscale, LEN, "%d", def_lossy_scale);
    ctl->met_lossy_scale[i] =
      (int) scan_ctl(filename, argc, argv, "MET_LOSSY_SCALE", i, defscale,
		     NULL);
    if (ctl->met_lossy_scale[i] < 0 || ctl->met_lossy_scale[i] > 1)
      ERRMSG("Set MET_LOSSY_SCALE to 0 or 1!");
  }
...
void read_met_bin_3d(
  ...
  /* Read packed data... */
    ...
    compress_pck(ctl, met, varname, help, 1, NULL, in);
  ...
  /* Read ZFP data... */
    ...
    compress_zfp(ctl, met, varname, help, 1, NULL, in);
#else
    ERRMSG("MPTRAC was compiled without ZFP compression!");
#endif
  }
...
#ifdef ZSTD
    compress_zstd(ctl, met, varname, help, 1, NULL, in);
#else
    ERRMSG("MPTRAC was compiled without ZSTD compression!");
#endif
  }
...
#ifdef LZ4
    compress_lz4(ctl, met, varname, help, 1, NULL, in);
#else
    ERRMSG("MPTRAC was compiled without LZ4 compression!");
#endif
  }
...
#ifdef CMS
    compress_cms(ctl, met, varname, help, 1, NULL, in);
#else
    ERRMSG("MPTRAC was compiled without cmultiscale compression!");
#endif
  }
  ...
  /* Read SZ3 data... */
    ...
    compress_sz3(ctl, met, varname, help, 1, NULL, in);
#else
    ERRMSG("MPTRAC was compiled without sz3 compression!");
#endif
  }
...
int read_met_nc(
  ...
  /* Set coordinate system of meteo data... */
  met->coord_type = ctl->met_coord_type;
...
void dd_read_met_nc_grid(
  ...
  /* Get grid dimensions... */
  NC_INQ_DIM("lon", &dd->nx_glob, 0, 0, 0);
  NC_INQ_DIM("lat", &dd->ny_glob, 0, 0, 0);

  LOG(2, "Number of longitudes: %d", dd->nx_glob);
  LOG(2, "Number of latitudes: %d", dd->ny_glob);

  /* Check grid... */
  if (dd->nx_glob > DD_EX_GLOB || dd->ny_glob > DD_EY_GLOB)
    ERRMSG("Global grid is too large!");

  if (ctl->dd_subdomains_zonal > dd->nx_glob)
    ERRMSG("Too many zonal subdomains for global x grid!");

  if (ctl->dd_subdomains_meridional > dd->ny_glob)
    ERRMSG("Too many meridional subdomains for global y grid!");

  /* Read global longitudes and latitudes... */
  NC_GET_DOUBLE("lon", dd->lon_glob, 1);
  NC_GET_DOUBLE("lat", dd->lat_glob, 1);

  LOG(2, "Longitudes: %g, %g ... %g deg",
      dd->lon_glob[0], dd->lon_glob[1], dd->lon_glob[dd->nx_glob - 1]);
  LOG(2, "Latitudes: %g, %g ... %g deg",
      dd->lat_glob[0], dd->lat_glob[1], dd->lat_glob[dd->ny_glob - 1]);

  /* Rank coordinates in DD layout... */
  const int zonal_rank = rank / ctl->dd_subdomains_meridional;
  const int merid_rank = rank % ctl->dd_subdomains_meridional;

  /* Check for edge cases... */
  const int left = (zonal_rank == 0);
  const int right = (zonal_rank == ctl->dd_subdomains_zonal - 1);
  const int top = (merid_rank == 0);
  const int bottom = (merid_rank == ctl->dd_subdomains_meridional - 1);

  /* Core owned block (without halos)... */
  const int nx_block = dd->nx_glob / ctl->dd_subdomains_zonal;
  const int ny_block = dd->ny_glob / ctl->dd_subdomains_meridional;
  ...
  int nx_core = nx_block;
  int ny_core = ny_block;

  if (right)
    nx_core += dd->nx_glob - ctl->dd_subdomains_zonal * nx_block;
  if (bottom)
    ny_core += dd->ny_glob - ctl->dd_subdomains_meridional * ny_block;

  /* Store core met size first... */
  met->nx = nx_core;
  met->ny = ny_core;

  /* Set hyperslab for core subdomain... */
  dd->subdomain_start[0] = 0;
  dd->subdomain_start[1] = 0;
  dd->subdomain_start[2] = (size_t) iy0;
  dd->subdomain_start[3] = (size_t) ix0;

  dd->subdomain_count[0] = 1;
  dd->subdomain_count[1] = (size_t) met->np;
  dd->subdomain_count[2] = (size_t) ny_core;
  dd->subdomain_count[3] = (size_t) nx_core;

  /* Add inner halos to read window... */
  if (!left && !right) {
  ...
  if (left ^ right) {
    ...
    dd->halo_bnd_start[3] =
      (size_t) (left ? (dd->nx_glob - ctl->dd_halos_size) : 0);
    ...
    dd->halo_offset_start = 0;
    dd->halo_offset_end = 0;
  }

  /* Focus on subdomain latitudes... */
  for (int iy = 0; iy < (int) dd->subdomain_count[2]; iy++)
    met->lat[iy] = dd->lat_glob[(int) dd->subdomain_start[2] + iy];

  /* Focus on subdomain longitudes... */
  for (int ix = 0; ix < (int) dd->subdomain_count[3]; ix++)
    met->lon[ix + dd->halo_offset_start] =
      dd->lon_glob[(int) dd->subdomain_start[3] + ix];

  for (int ix = 0; ix < (int) dd->halo_bnd_count[3]; ix++)
    met->lon[ix + dd->halo_offset_end] =
      dd->lon_glob[(int) dd->halo_bnd_start[3] + ix] + lon_shift;

  /* Reset halo-extended grid dimensions... */
  ...
  LOG(2, "Edge position: l=%d,r=%d,t=%d,b=%d", left, right, top, bottom);
  LOG(2, "Total size for subdomain meteo data: nx %d ny %d np %d",
      met->nx, met->ny, met->np);
  ...
  LOG(2, "Subdomain start: nx %ld ny %ld np %ld",
      dd->subdomain_start[3], dd->subdomain_start[2], dd->subdomain_start[1]);
  LOG(2, "Boundary halo start: nx %ld ny %ld np %ld",
      dd->halo_bnd_start[3], dd->halo_bnd_start[2], dd->halo_bnd_start[1]);
  LOG(2, "Offsets: nx %d ny %d", dd->halo_offset_start, dd->halo_offset_end);
  LOG(2, "%d Subdomain longitudes: %g, %g ... %g deg",
      rank, met->lon[0], met->lon[1], met->lon[met->nx - 1]);
  LOG(2, "%d Subdomain latitudes: %g, %g ... %g deg",
      rank, met->lat[0], met->lat[1], met->lat[met->ny - 1]);
}
...
#pragma omp parallel for default(shared) collapse(2)
  ...
  /* Write packed data... */
    ...
    compress_pck(ctl, met, varname, help, 0, level_log, out);

  /* Write ZFP data... */
#ifdef ZFP
  else if (ctl->met_type == 3) {
    FWRITE(&ctl->met_zfp_prec[metvar], int,
	   1,
	   out);
    FWRITE(&ctl->met_zfp_tol[metvar], double,
	   1,
	   out);
    compress_zfp(ctl, met, varname, help, 0, level_log, out);
  }
#endif
  ...
  /* Write zstd data... */
    ...
    compress_zstd(ctl, met, varname, help, 0, level_log, out);
#endif

  /* Write LZ4 data... */
#ifdef LZ4
  else if (ctl->met_type == 8)
    compress_lz4(ctl, met, varname, help, 0, level_log, out);
#endif

  /* Write cmultiscale data... */
#ifdef CMS
  else if (ctl->met_type == 5) {
    compress_cms(ctl, met, varname, help, 0, level_log, out);
  }
#endif
  ...
  /* Write SZ3 data... */
    ...
    FWRITE(&ctl->met_sz3_prec[metvar], int,
	   1,
	   out);
    FWRITE(&ctl->met_sz3_tol[metvar], double,
	   1,
	   out);
    compress_sz3(ctl, met, varname, help, 0, level_log, out);
  }
#endif
