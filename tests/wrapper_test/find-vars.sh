...
# extract structures
cat ../../src/mptrac_fortran.f90| awk '/TYPE, bind\(c\) :: ctl_t/{f=1;next} /END TYPE ctl_t/{f=0} f' > data/test_ctl_f.asc
cat ../../src/mptrac_fortran.f90| awk '/TYPE, bind\(c\) :: met_t/{f=1;next} /END TYPE met_t/{f=0} f' > data/test_met_f.asc
cat ../../src/mptrac_fortran.f90| awk '$0=="  TYPE, bind(c) :: clim_t"{f=1;next} /END TYPE clim_t/{f=0} f'  > data/test_clim_f.asc
cat ../../src/mptrac_fortran.f90| awk '/TYPE, bind\(c\) :: clim_zm_t/{f=1;next} /END TYPE clim_zm_t/{f=0} f' > data/test_clim_zm_f.asc
cat ../../src/mptrac_fortran.f90| awk '/TYPE, bind\(c\) :: clim_ts_t/{f=1;next} /END TYPE clim_ts_t/{f=0} f' > data/test_clim_ts_f.asc
cat ../../src/mptrac_fortran.f90| awk '/TYPE, bind\(c\) :: clim_photo_t/{f=1;next} /END TYPE clim_photo_t/{f=0} f' > data/test_clim_photo_f.asc

cat  ../../src/mptrac.h| awk '/Control parameters/{f=1;next} /\} ctl_t;/{f=0} f' > data/test_ctl_c.asc
cat  ../../src/mptrac.h| awk '/Meteo data structure/{f=1;next} /\} met_t;/{f=0} f' > data/test_met_c.asc
cat  ../../src/mptrac.h| awk '/climatological data containing/{f=1;next} /\} clim_t;/{f=0} f' > data/test_clim_c.asc
cat  ../../src/mptrac.h| awk '/Climatological data in the form of zonal means./{f=1;next} /\} clim_zm_t;/{f=0} f' > data/test_clim_zm_c.asc
cat  ../../src/mptrac.h| awk '/Climatological data in the form of time series./{f=1;next} /\} clim_ts_t;/{f=0} f' > data/test_clim_ts_c.asc
cat  ../../src/mptrac.h| awk '/Climatological data in the form of photolysis rates./{f=1;next} /\} clim_photo_t;/{f=0} f' > data/test_clim_photo_c.asc

# compare number of structure entries and variables inside 
cat data/test_ctl_c.asc| grep ";"| wc -l > x
cat data/test_ctl_f.asc|wc -l > y
...
cat data/test_met_c.asc| grep ";"| wc -l > x
cat data/test_met_f.asc|wc -l > y
...
cat data/test_clim_c.asc| grep ";"| wc -l > x
cat data/test_clim_f.asc|wc -l > y
...
cat data/test_clim_zm_c.asc| grep ";"| wc -l > x
cat data/test_clim_zm_f.asc|wc -l > y
...
cat data/test_clim_ts_c.asc| grep ";"| wc -l > x
cat data/test_clim_ts_f.asc|wc -l > y
...
cat data/test_clim_photo_c.asc| grep ";"| wc -l > x
cat data/test_clim_photo_f.asc|wc -l > y
...
if cmp -s x y && cmp -s data/var_clim_photo_c.asc data/var_clim_photo_f.asc ; then
    ...
    echo "Number of climatological datain the form of photolysis rates are not equal. C has: "
    cat x
    echo "and Fortran has: "
    cat y
