...
# Check arguments...
if [ $# -ne 15 ] ; then
    echo "usage: $0 <system> <trac> <libs> <compiler> <gpu> <npmin> <npmax> <npfac> <meteo> <metdir> <rng> <sort> <phys> <cache> <refdir>"
    exit
fi
...
if [ "$compiler" = "gcc" ] ; then
    ml GCC ParaStationMPI git
    echo -e "\ngcc version:" ; gcc --version
elif [ "$compiler" = "nvc" ] ; then
    ml NVHPC ParaStationMPI git
...
# Create directories...
rm -rf data && mkdir -p data logs || exit
...
for np in $(echo $npmin $npmax $npfac | awk '{for(np=$1; np<=$2; np*=$3) print np}') ; do
	...
	[ "$meteo" = "erai" ] && metbase="METBASE $metdir/erai_pck/ei DT_MET 21600 DT_MOD 360 MET_TYPE 2"
	[ "$meteo" = "era5" ] && metbase="METBASE $metdir/era5_pck/era5 DT_MET 3600 DT_MOD 180 MET_TYPE 2"
	...
	for var in mean stddev absdev median mad ; do
	    $trac/src/atm_dist - $dir/dist.tab $var \
			       $dir/1/atm_2017_01_01_00_00.bin $refdir/$meteo/phys_${phys}/$np/1/atm_2017_01_01_00_00.bin \
			       $dir/1/atm_2017_01_01_06_00.bin $refdir/$meteo/phys_${phys}/$np/1/atm_2017_01_01_06_00.bin \
			       $dir/1/atm_2017_01_01_12_00.bin $refdir/$meteo/phys_${phys}/$np/1/atm_2017_01_01_12_00.bin \
			       $dir/1/atm_2017_01_01_18_00.bin $refdir/$meteo/phys_${phys}/$np/1/atm_2017_01_01_18_00.bin \
			       $dir/1/atm_2017_01_02_00_00.bin $refdir/$meteo/phys_${phys}/$np/1/atm_2017_01_02_00_00.bin \
			       $qnt ATM_TYPE 1
	...
	# Compare grid files...
	echo -e "\nGrid differences..."
	paste $dir/1/grid_2017_01_02_00_00.tab "$refdir/$meteo/phys_${phys}/$np/1/grid_2017_01_02_00_00.tab" | awk '{
	  if(NF>0 && $1!="#") {
            cmax=NF/2
            for(c=1; c<=cmax; c++) {
